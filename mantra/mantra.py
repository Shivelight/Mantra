...
from .service.ttypes import (
    ...
    TalkException, AccountMigrationCheckType, DeviceInfo, CarrierCode
)
...
class Mantra(LineApi):
    ...
    def __init__(self, loop=None, session=None, geo='JP'):
        ...
        if session is None:
            tcp = TCPConnector(ssl=False, loop=self.loop)
            skip_headers = ['Accept-Encoding']
            self.session = ClientSession(loop=self.loop, connector=tcp,
    ...
    async def login(self, identity=None, password=None, cert=None, token=None,
                    qr=False, key=None, callback=print, clientType="win10",
                    protocol="binary", sns=None, ssl=True, **kw):

        url = ("https://" if ssl else "http://") + self.LEGY

        self.Auth = client.auth(url + SERVICE[protocol]['REGISTRATION'],
        ...
        self.Talk = client.talk(url + SERVICE[protocol][talkpath],
                                self.session, protocol)

        self.Poll = client.talk(url + SERVICE[protocol]['LONG_POLLING'],
                                self.session, protocol)
        self.Poll.trans.updateCustomHeaders({'Connection': 'close'})

        self.Channel = client.channel(url + SERVICE[protocol]['CHANNEL'],
                                      self.session, protocol)
        self.Call = client.call(url + SERVICE[protocol]['CALL'],
                                self.session, protocol)
    ...
    async def _login(self, identity, password, cert=None, callback=print):
        ...
        message = (chr(len(rsainfo.sessionKey)) + rsainfo.sessionKey +
                   chr(len(identity)) + identity +
                   chr(len(password)) + password).encode('utf-8')
        publicKey = rsa.PublicKey(int(rsainfo.nvalue, 16),
                                  int(rsainfo.evalue, 16))
        crypto = rsa.encrypt(message, publicKey).hex()

        self.Auth.trans.path = SERVICE[self.protocol]['AUTH_REGISTRATION']

        loginRequest = LoginRequest(LoginType.ID_CREDENTIAL,
                                    identityProvider,
                                    rsainfo.keynm,
                                    crypto,
                                    True,
                                    "127.0.0.1",
                                    "Mantra", cert, None, None, 0)
        ...
        if loginResult.type == LoginResultType.SUCCESS:
            ...
            header = {"X-Line-Application": APP[self.clientType]['LineApplication'],
            ...
            loginRequest = LoginRequest(LoginType.QRCODE, keepLoggedIn=True,
    ...
    async def qrLogin(self, callback=print, keyLogin=False):
        ...
        loginRequest = LoginRequest(LoginType.QRCODE,
    ...
    async def loginWithSns(self, snsIdType, accessToken, region="JP"):
        ...
        if userStatus.accountMigrationCheckType == AccountMigrationCheckType.SKIP:
            deviceInfo = DeviceInfo("SM-A310F", "Android OS", "7.0",
                                    "SM-A310F", 0, "NOT_SPECIFIED", None)
            registration = await self.Auth.registerWithSnsId(
                snsIdType, accessToken, region, udidHash, deviceInfo, None,
                session)
    ...
    async def registerWithPhone(self, phone, region):
        ...
        verification = await self.Auth.startVerification(
            region, CarrierCode.NOT_SPECIFIED, phone, udidHash, deviceInfo,
            "51011", None, "en_GB", None, oldUdidHash)
        pinCode = input("Pincode: ")
        await self.Auth.verifyPhoneNumber(verification.sessionId, pinCode,
                                          udidHash, oldUdidHash)
